  ...
  <section class="optimizer-overview">
    ...
    <div class="optimizer-overview__stats-grid">
      <UiCard class="optimizer-overview__stat">
        ...
        <span v-if="poolIsEmpty" class="optimizer-overview__stat-note"
          >No eval pool cases yet.</span
        >
      </UiCard>
      <UiCard class="optimizer-overview__stat">
    ...
    <div class="optimizer-overview__main-grid">
      <UiCard class="optimizer-overview__panel">
        ...
        <div v-if="poolIsEmpty" class="optimizer-overview__empty">
          No eval pool cases yet. Import a dataset or review pending relabeled cases before running
          pool evaluation.
        </div>
        <div v-else class="optimizer-overview__coverage-list">
          <div
            v-for="action in ARBITER_ACTIONS"
            :key="action"
            class="optimizer-overview__coverage-row"
          >
            ...
            <div class="optimizer-overview__segments">
              <span
                ...
                :class="{
                  'optimizer-overview__segment--active': segment <= activeSegments(action),
                }"
              />
            </div>
            <span
      ...
      <UiCard class="optimizer-overview__panel">
        ...
        <div v-if="props.optimizerDefaultModel" class="optimizer-overview__model-info">
          ...
          <code class="optimizer-overview__model-info-value">{{
            props.optimizerDefaultModel
          }}</code>
        </div>
        <UiButton
...
.optimizer-overview__run-banner > div {
  display: flex;
  align-items: center;
  gap: 10px;
}
.optimizer-overview__stats-grid {
  display: grid;
  grid-template-columns: repeat(4, minmax(0, 1fr));
  gap: 12px;
}
.optimizer-overview__stat {
  display: flex;
  flex-direction: column;
  gap: 6px;
}
.optimizer-overview__stat-value {
  color: var(--fg-0);
  font-size: 26px;
  font-weight: 700;
  line-height: 1;
}
.optimizer-overview__stat-note,
.optimizer-overview__muted {
  color: var(--fg-4);
  font-size: 12px;
}
.optimizer-overview__main-grid {
  display: grid;
  grid-template-columns: minmax(0, 1.4fr) minmax(320px, 0.8fr);
  gap: 14px;
}
.optimizer-overview__panel {
  display: flex;
  flex-direction: column;
  gap: 14px;
}
.optimizer-overview__panel-head {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
}
.optimizer-overview__coverage-list {
  display: flex;
  flex-direction: column;
  gap: 10px;
}
.optimizer-overview__coverage-row {
  display: grid;
  grid-template-columns: 150px minmax(0, 1fr) 42px;
  gap: 10px;
  align-items: center;
}
.optimizer-overview__segments {
  display: grid;
  grid-template-columns: repeat(10, 1fr);
  gap: 3px;
}
.optimizer-overview__segment {
  height: 8px;
  border-radius: var(--r-pill);
  background: var(--bg-2);
}
.optimizer-overview__segment--active {
  background: var(--accent);
}
.optimizer-overview__coverage-count {
  color: var(--fg-3);
  text-align: right;
}
.optimizer-overview__coverage-count--low {
  color: var(--warning);
}
.optimizer-overview__form-grid {
  display: grid;
...
.optimizer-overview__wide-btn {
  width: 100%;
  justify-content: center;
}
.optimizer-overview__empty {
  display: flex;
  justify-content: center;
...
.optimizer-overview__table {
  width: 100%;
  border-collapse: collapse;
  margin-top: 12px;
}
.optimizer-overview__table th,
.optimizer-overview__table td {
...
.optimizer-overview__click-row {
  cursor: pointer;
}
.optimizer-overview__click-row:hover td {
  background: var(--bg-2);
}
