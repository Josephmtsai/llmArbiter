...
// Inline multi-statement handlers lose their separator under Prettier's
// semi:false and the template compiler then cannot parse them.
function prevPage() {
  page.value--
  load()
}
  ...
  <div class="arb-decisions">
    ...
    <div v-if="stats" class="arb-decisions__stats-section">
      ...
      <div class="arb-decisions__stats">
        ...
        <UiCard v-for="action in PRIMARY_ACTIONS" :key="action" class="arb-decisions__stat">
    ...
    <div class="arb-decisions__table-wrap">
      ...
      <div v-else-if="decisions.length === 0" class="arb-decisions__empty">No decisions found.</div>
          ...
          >
            ...
            <td>
              <div class="arb-decisions__provider-cell">
                ...
                <span v-if="d.model !== undefined" class="arb-decisions__model-sub num">{{
                  d.model || '—'
                }}</span>
              </div>
            </td>
            <td class="num arb-decisions__td-time">
    ...
    <div v-if="!loading && decisions.length > 0" class="arb-decisions__pagination">
      <UiButton variant="ghost" size="sm" :disabled="page === 0" @click="prevPage()">
        Prev
      </UiButton>
        ...
        <div class="arb-drawer__body">
          <div class="arb-drawer__row">
            ...
            <div class="arb-drawer__provider-block">
              ...
              <span
                v-if="selectedDecision.model !== undefined"
                class="arb-decisions__model-sub num"
                >{{ selectedDecision.model || '—' }}</span
              >
            </div>
          </div>
          ...
          <div class="arb-drawer__section">
            ...
            <span class="arb-drawer__text num">{{
...
.arb-decisions__filter-btn:hover {
  background: var(--bg-2);
  color: var(--fg-1);
}
...
.arb-decisions__time-pill:hover {
  background: var(--bg-2);
  color: var(--fg-1);
}
...
.arb-decisions__row:hover td {
  background: var(--bg-2);
}
.arb-decisions__row:last-child td {
  border-bottom: none;
}
.arb-decisions__td-time {
  font-size: 11px;
  color: var(--fg-4);
}
